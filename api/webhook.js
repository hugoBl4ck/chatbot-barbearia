// =================================================================
// WEBHOOK PARA AGENDAMENTO DE BARBEARIA (VERSÃO FINAL E SEGURA)
// =================================================================
...
app.post("/api/webhook", async (request, response) => {
    ...
    try {
        ...
        console.log("Sessão recebida:", session); // Log de depuração sugerido
        ...
        // =========================================================
        // LÓGICA DE CONTEXTO CORRIGIDA E SEGURA
        // =========================================================
        if (resultPayload.success === false && intentName.startsWith("AgendarHorario")) {
            // Valida se a 'session' existe antes de tentar criar um contexto
            if (session) {
                const personInfo = getPersonInfo(outputContexts);
                ...
                console.warn("⚠️  Variável 'session' não recebida do Dialogflow. O contexto de remarcação não pôde ser ativado.");
            }
        }
        
        const duration = (Date.now() - startTime) / 1000;
        console.log(`⏱️ Tempo de Execução: ${duration.toFixed(2)} segundos`);
        console.log("📤 RESPOSTA JSON ENVIADA:", JSON.stringify(responseData, null, 2));
...
// =================================================================
// FUNÇÕES AUXILIARES (COLE O RESTANTE DO SEU CÓDIGO AQUI)
// ...
// =================================================================

// ... (Cole aqui o código completo das funções handleScheduling, checkConflicts, etc.)
// ... (O código delas já está correto)


// EXEMPLO DAS FUNÇÕES QUE VOCÊ DEVE MANTER:
async function handleScheduling(personInfo, dateTimeParam, db) {
    ...
    const [businessHoursCheck, hasConflict] = await Promise.all([checkBusinessHours(requestedDate, db), checkConflicts(requestedDate, db)]);
...
async function handleCancellation(personInfo, db) {
    ...
    const snapshot = await schedulesRef.where('TelefoneCliente', '==', personInfo.phone).where('Status', '==', 'Agendado').where('DataHoraISO', '>', new Date().toISOString()).orderBy('DataHoraISO', 'desc').limit(1).get();
    if (snapshot.empty) { return { success: false, message: `Não encontrei nenhum agendamento futuro no seu nome para cancelar.` }; }
...
async function checkBusinessHours(date, db) {
    ...
    const isWithinHours = (time, start, end) => { if (!start || !end) return false; const startTime = timeToDecimal(start); const endTime = timeToDecimal(end); return time >= startTime && (time + serviceDurationInHours) <= endTime; };
    ...
    else { const morning = `das ${dayConfig.InicioManha} às ${dayConfig.FimManha}`; const afternoon = dayConfig.InicioTarde ? ` e das ${dayConfig.InicioTarde} às ${dayConfig.FimTarde}` : ''; const message = `O horário que você sugeriu está fora do nosso expediente. Em ${dayName}, nosso horário é ${morning}${afternoon}. Qual horário nesse período você gostaria de marcar?`; return { isOpen: false, message: message }; }
}
async function checkConflicts(requestedDate, db) {
    ...
    const schedulesRef = db.collection(CONFIG.collections.schedules);
    const snapshot = await schedulesRef.where('Status', '==', 'Agendado').where('DataHoraISO', '>=', searchStart.toISOString()).where('DataHoraISO', '<=', searchEnd.toISOString()).get();
    ...
    for (const doc of snapshot.docs) {
        ...
        if ((requestedStart < existingEnd) && (requestedEnd > existingStart)) { console.log(`💥 CONFLITO ENCONTRADO com agendamento das ${doc.data().DataHoraISO}`); return true; }
    }
    return false;
}
async function saveAppointment(personInfo, requestedDate, db) {
    const newAppointment = { NomeCliente: personInfo.name, TelefoneCliente: personInfo.phone, DataHoraISO: requestedDate.toISOString(), DataHoraFormatada: new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium', timeStyle: 'short', timeZone: CONFIG.timezone }).format(requestedDate), Status: 'Agendado', TimestampAgendamento: new Date().toISOString() };
...
function getPersonInfo(contexts) {
    ...
    for (const context of contexts) {
        ...
        if (params) { info.name = info.name || params.person?.name || params.nome; info.phone = info.phone || params.telefone || params['phone-number']; }
    }
    return info;
}
function extractDateFromDialogflow(param) {
    ...
    else if (typeof param === 'object' && param !== null) { dateString = param.startDateTime || param.date_time || param.date; }
    if (dateString && typeof dateString === 'string') { const date = new Date(dateString); return isNaN(date.getTime()) ? null : date; }
    return null;
}
