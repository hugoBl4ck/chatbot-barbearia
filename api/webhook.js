// =================================================================
// WEBHOOK MULTI-TENANT - VERSÃO CORRIGIDA
// =================================================================
...
// A função parseDateTime permanece a mesma
...
app.post("/api/webhook", async (request, response) => {
    ...
    try {
        const { intent, nome, telefone, data_hora_texto, servicoId, barbeariaId } = body;
        const db = admin.firestore();
        let resultPayload;

        if (!barbeariaId) {
            return response.status(400).json({ status: 'error', message: "ID da barbearia não foi fornecido no webhook." });
        }
        ...
        if (intent === 'agendarHorario') {
            ...
            if (!parsedDate) {
                ...
                resultPayload = await handleScheduling(barbeariaId, personInfo, dateForStorage, dateForValidation, servicoId, db);
            }
...
async function handleScheduling(barbeariaId, personInfo, requestedDate, localTime, servicoId, db) {
    ...
    const duracao = servico.duracaoMinutos || 30; // Fallback para 30 min
...
async function getAvailableSlots(barbeariaId, requestedDate, duracaoMinutos, db) {
    try {
        const requestedDateDayjs = dayjs(requestedDate).tz(CONFIG.timezone);
        
        let availableSlots = await findAvailableSlotsForDay(barbeariaId, requestedDateDayjs, duracaoMinutos, db);
...
async function findAvailableSlotsForDay(barbeariaId, dayDate, duracaoMinutos, db) {
    ...
    const addSlotsFromPeriod = (startMinutes, endMinutes) => {
        ...
        for (let time = startMinutes; time + duracaoMinutos <= endMinutes; time += 30) { // Assumindo slots de 30 min
...
async function saveAppointment(barbeariaId, personInfo, requestedDate, servico, db) {
    const schedulesRef = db.collection(CONFIG.collections.barbearias).doc(barbeariaId).collection(CONFIG.collections.schedules);
